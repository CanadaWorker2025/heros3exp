...
后续修复（2026-09-26，当前未提交）：
- `extraCast` 与 `levelCast` 原先没有独立 `stacking` 标识，同一英雄特长来源下被奖励系统归为同一叠加组，只保留一个 `+1`。
- 现分别增加 `aegrenalExtraCast` 与 `aegrenalLevelCast`，使固定额外施法和等级成长施法可以相加。
- 预期次数为：1-14级每回合2次，15-29级3次，30-44级4次，之后每15级继续增加1次。
